...
- [ ] Add support for implicitly handling git operation. Any generation command should automtically create git branches in the output paths with a common naming convention/pattern. You can assume that each out put path is already a git repository with a remote configured. Then, at the completion of the generation process, commit to that branch and push to the remote. PRs will be handled manually by the user.
- [x] At the completion of the generation process, output a summary of what was generated, similar to how kubectl outputs a summary after applying manifests. This should include the file paths and types of artifacts generated. (Note: git status feature deferred pending complete requirements)
